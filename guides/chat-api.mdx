...
|-------|------|-------------|
| `stream_start` | `{ "agent_name": "Support Agent" }` | Streaming started |
| `chunk` | `{ "delta": "Hello! " }` | Incremental text chunk |
| `tool_call` | `{ "name": "web_search", "status": "executing" }` | Tool execution started |
| `tool_result` | `{ "name": "web_search", "status": "completed" }` | Tool execution completed |
| `handoff` | subagent details | Multi-agent handoff occurring |
| `stream_end` | `{ "usage": {"input_tokens": N, "output_tokens": N}, "end_session": false }` | Response complete |
